...
must_haves:
  ...
  key_links:
    ...
    - from: "~/.claude/skills/natal-chart/SKILL.md"
      ...
      via: "Read tool to load profile data into context"
      pattern: "chart\\.json"
---
...
If any test fails, update the SKILL.md to match actual backend behavior (correct command syntax, adjust error messages, fix path references). The skill must accurately reflect how the backend actually works.
  ...
  <verify>
All 6 tests pass with expected exit codes and outputs. Test profiles cleaned up. If any SKILL.md corrections were needed, verify the file still passes Task 1 verification checks.
  </verify>
  <done>
